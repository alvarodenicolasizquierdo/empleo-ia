...
### ✅ CURRENT STATE: v14 dataset / v29a methodology (v29a = v29 + Lovable UI deployment: 4 UI/PRODUCT issues FIXED (#20/#22/#24/#25), scoreboard 24/28; v29 = v28 + Appendix F terminology alignment; v28 = v27 + 3rd red team mapping + Appendix F/JSX v14 update; v27 = v26 + Appendix F Karpathy + notes [48-56]; v14 = v13 + admin-override patch for 5 security/public-service occupations)
...
| 5 | Version | PDF filename vs header vs Zenodo version chaos | CRITICAL | DEFECT | 🟢 **FIXED (v22)** — Version table in Section 12 maps v17→v29a. All headers consistent. Zenodo still says v17. | Update Zenodo when uploading final | 🟢 FIXED |
...
| Category | Count | Issues |
|----------|-------|--------|
| FIXED | 6 | 18, 19, 20, 22, 24, 25 |
| UI/PRODUCT (not methodology) | 1 | 27 (salary backbone — blocked on MCVL data access) |
| LIMITATION (documented) | 2 | 21, 26 |
| PROCESS/HYGIENE | 2 | 23, 28 |
| **TOTAL RESOLVED** | **8 of 11** | (6 FIXED + 2 LIMITATION) |

**Combined scoreboard: 24 of 28 resolved** (16/17 original + 8/11 3rd red team)
...
| 5 | 🔴 CRITICAL DEFECT (version chaos) | 🟢 FIXED | v22 has version mapping table. Now extends to v29a. |
...
|---|------|-------|-------------|------------------------------|------------------|---------------|
| 18 | Packaging | Cross-artifact regression in public-facing assets. Zenodo v26 points to old Netlify URL, lists v23 reproducibility ZIP, truncates field description. Standalone v26 HTML contains stale "Nuestro 20,9%" and old Zenodo DOI. | CRITICAL | 🟢 **FIXED (v28/v29)** — Zenodo description updated for v27/v14. Reproducibility ZIP updated to v27. Standalone HTML and production page synchronized. Appendix F and JSX artifact updated with v14 data + terminology alignment (v29). All public-facing numbers now reference 22.73M/12.1%/56 notes. | None | 🟢 FIXED |
...
| 20 | UI/Comms | Treemap screenshot risk around the wage index. UI foregrounds large euro figure per occupation while "not money at risk" caveat sits in footer text only. | SIGNIFICANT | 🟢 **FIXED (v29a)** — Lovable UI deployment: KPI ribbon now shows "Indicador teórico — no euros en riesgo" inline beneath the 252.8B € figure. Detail panel repeats same caveat. Methodology PDF disclaimer (Section 5) remains. | None | 🟢 FIXED |
...
| 22 | Legal/UI | EU AI Act badge still creates a public-facing paradox. AI Act classifies systems by use case, not occupations; the UI badge next to the exposure score creates "blue score / red risk" confusion. | SIGNIFICANT | 🟢 **FIXED (v29a)** — Lovable UI deployment: EU AI Act badge separated into own "Contexto regulatorio" section in detail panel with explainer "Proxy regulatorio de contexto de uso, no dictamen legal." Header caveat pill: "EU AI Act = contexto regulatorio, no riesgo laboral." Full disclosure section "Qué significa EU AI Act aquí" in methodology panel. | None | 🟢 FIXED |
| 23 | Process | Working doc (Google Doc) is stale and misdirecting. Title still says "Draft 2_RedTeam_Adversarial_Review_v19." | MINOR | 🟢 **ADDRESSED (v29a)** — Status Updates 6, 7, 8, 9, and 10 added documenting v26→v27→v28→v29→v29a progression. Title reflects historical origin; status updates provide current context. | Consider adding deprecation banner | 🟢 ADDRESSED |
| 24 | UI | employment_confidence is hidden in the UI. 20.96% of employment sits in Confidence B occupations, yet the field is not surfaced to users. | SIGNIFICANT | 🟢 **FIXED (v29a)** — Lovable UI deployment: "Calidad de la estimación" section in detail panel shows confidence tier (A+/A/B) with colored badge and explanation. Tier B explicitly labeled "Estimación más dependiente de pesos sintéticos SEPE." Validation stats (κ, n, coverage) displayed inline. | None | 🟢 FIXED |
| 25 | Score/UI | Sustitución label on sub-7 occupations is semantically sloppy. 15 occupations verified with Sustitución label and scores below 7. | SIGNIFICANT | 🟢 **FIXED (v29a)** — Lovable UI deployment: Labels refined — "Sustitución parcial de tareas" for high-score occupations (with clarification "Se refiere a presión de automatización sobre tareas, no necesariamente al puesto completo"), "Híbrido" for borderline/intermediate scores ("Combinación de automatización parcial y aumentación de capacidades"). Sub-7 occupations no longer show bare "Sustitución." | None | 🟢 FIXED |
...
| 28 | Process | Issue tracker contained stale numbers / stale sense of closure. Presented as 15/16 resolved with v23 ZIP reference. | MINOR | 🟢 **ADDRESSED (v29a)** — Issue tracker fully updated to 28 issues. Scoreboard reflects v29a/v14 (24/28 resolved). 3rd red team issues explicitly mapped. | None | 🟢 ADDRESSED |
...
2. ✅ Standalone/live HTML updated with current numbers (22.73M/12.1%) and Zenodo V28 DOI link (#18 — v29a)
3. ✅ Police/security/prison employment anomalies are overridden (#19 via v14)
4. ✅ Wage index and EU AI Act badge carry inline explanatory copy (#20, #22 — Lovable UI v29a)
